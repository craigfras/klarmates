/**
 * POST /api/admin/week/restart — revert the current open week to questions-review.
 *
 ...
 * gameService.restartWeek. Auth failures → 403; malformed body /
...
export async function POST(request: Request): Promise<Response> {
  ...
  // --- Safe body parse (malformed JSON → 400, no restartWeek call) --------
  let weekId: string;
  try {
  ...
  try {
    await gameService.restartWeek(weekId);
